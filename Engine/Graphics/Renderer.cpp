...
namespace dwb
{
	...
	void Renderer::Draw(std::shared_ptr<dwb::Texture> texture, const SDL_Rect& source, const Transform& transform)
	{
		...
		SDL_RenderCopyEx(renderer, texture->texture, &source, &dest, dwb::RadToDeg(transform.rotation), nullptr, SDL_FLIP_NONE);
	}
}
